...
async def main():
    ...
    # This clears 'daily_games' table and inserts today's schedule
    from app.scrappers.insert_tables import populate_daily_games
    await populate_daily_games(daily_df)

    # 3) Process historical games (yesterday and before)
    if not historical_df.empty:
        # Generate a DataFrame with full boxscore stats
        boxscores_df = process_games_from_schedule_df(historical_df)

        # Insert historical data into the database (games, team_stats, player_stats)
